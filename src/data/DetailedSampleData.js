export const detailedRecipes = [
    {
        imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
        title: "Cheese Noodles",
        time: "70min",
        price: "4.99",
        tags: ["Vegetarian", "Fast"],
        ingredients: [
            { amount: "200g", name: "Spätzle" },
            { amount: "150g", name: "Cheese" },
            { amount: "1", name: "Onion" },
        ],
        nutrition: {
            calories: "600kcal",
            fat: "22g",
            protein: "20g",
            sugar: "8g",
        },
        steps: [
            {
                imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
                description: "Boil the Spätzle in salted water until tender.",
            },
            {
                imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
                description: "Fry the onions until golden and crispy.",
            },
            {
                imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
                description: "Mix the Spätzle with cheese and top with onions.",
            },
        ],
    },
    {
        imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
        title: "High Protein Chocolate Cake",
        time: "30min",
        price: "3.49",
        tags: ["Simple", "Recommended", "Dessert"],
        ingredients: [
            { amount: "100g", name: "Protein powder" },
            { amount: "50g", name: "Cocoa powder" },
            { amount: "2", name: "Eggs" },
        ],
        nutrition: {
            calories: "450kcal",
            fat: "10g",
            protein: "35g",
            sugar: "12g",
        },
        steps: [
            {
                imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
                description: "Mix all ingredients into a batter.",
            },
            {
                imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
                description: "Pour into baking form and bake for 20 minutes.",
            },
        ],
    },
    {
        imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
        title: "Caesar Salad",
        time: "25min",
        price: "2.99",
        tags: ["Vegetarian", "Healthy", "Recommended"],
        ingredients: [
            { amount: "1", name: "Romaine lettuce" },
            { amount: "50g", name: "Parmesan" },
            { amount: "30g", name: "Croutons" },
        ],
        nutrition: {
            calories: "300kcal",
            fat: "15g",
            protein: "10g",
            sugar: "4g",
        },
        steps: [
            {
                imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
                description: "Chop the lettuce and place in a bowl.",
            },
            {
                imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
                description: "Add dressing, parmesan, and croutons.",
            },
        ],
    },
    {
        imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
        title: "Avocado Toast",
        time: "5min",
        price: "1.49",
        tags: ["Vegetarian", "Fast"],
        ingredients: [
            { amount: "2 slices", name: "Bread" },
            { amount: "1", name: "Avocado" },
            { amount: "To taste", name: "Salt and pepper" },
        ],
        nutrition: {
            calories: "250kcal",
            fat: "14g",
            protein: "6g",
            sugar: "2g",
        },
        steps: [
            {
                imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
                description: "Toast the bread slices.",
            },
            {
                imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
                description: "Mash avocado and spread over toast.",
            },
        ],
    },
    {
        imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
        title: "Healthy Bowl",
        time: "40min",
        price: "5.49",
        tags: ["Vegetarian", "Healthy", "Recommended"],
        ingredients: [
            { amount: "100g", name: "Quinoa" },
            { amount: "1", name: "Avocado" },
            { amount: "50g", name: "Chickpeas" },
        ],
        nutrition: {
            calories: "480kcal",
            fat: "18g",
            protein: "16g",
            sugar: "6g",
        },
        steps: [
            {
                imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
                description: "Cook quinoa and prepare toppings.",
            },
            {
                imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
                description: "Assemble everything in a bowl.",
            },
        ],
    },
    {
        imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
        title: "Breaded Veal Cutlet",
        time: "25min",
        price: "6.99",
        tags: ["Simple", "Meat", "Recommended"],
        ingredients: [
            { amount: "1", name: "Veal cutlet" },
            { amount: "1", name: "Egg" },
            { amount: "50g", name: "Breadcrumbs" },
        ],
        nutrition: {
            calories: "520kcal",
            fat: "28g",
            protein: "25g",
            sugar: "1g",
        },
        steps: [
            {
                imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
                description: "Coat the veal cutlet in egg and breadcrumbs.",
            },
            {
                imageUrl: "https://www.kuechengoetter.de/uploads/media/630x630/08/71718-kaesespaetzle-mit-knusperzwiebeln.jpg?v=3-33",
                description: "Fry in hot oil until golden brown.",
            },
        ],
    },
];